...
export default function DestinationTable({ color }) {
  ...
  const deleteHandler = (id) => {
    ...
    if (confirm("Are you sure you want to delete this university?")) {
      fetch("/api/admin/university/?id=" + id, requestOptions)
        .then((response) => response.json())
        .then((result) => {
          console.log(result);
          router.reload();
  ...
  return (
    <>
      {loading && <PageChange path={"University"} />}
      <div
        className={
              ...
              {dataDescription.map((x, i) => (
                ...
                >
                  ...
                  <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                    {Array.isArray(x.courses)
                      ? x.courses
                          .slice(0, 3)
                          .map((course, i) => (
                            <p key={i}>{course.course_name}</p>
                          ))
                      : (() => {
                          try {
                            const parsedCourses = JSON.parse(x.courses || "[]");
                  ...
                  <td
                    ...
                    title={x.university_details}
                  >
                    <div
                      className="prose"
                      dangerouslySetInnerHTML={{ __html: x.university_details }}
                    />
                  </td>
                  <td
                    className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4"
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}>
        ...
        <p className="text-gray-700 mb-6">
          <UploadUniversity data={isSelected} edit={true} />
        </p>
        <button
